...
reg_counts <- CONS_C3_mod_c_a |> 
  mutate(
    ...
    region_clean = dplyr::recode(
      ...
      "magallanes y la antartica chilena" = "magallanes y de la antartica chilena",
  ...
  mutate(
    region_clean = stringr::str_to_lower(region_clean) |>
      stringr::str_replace_all("la ", "") |>
      stringr::str_replace_all("de la ", "") |>
      stringr::str_replace_all(" del ", " ") |>
      stringr::str_replace_all("libertador general bernardo o.?hi.*", "libertador general bernardo ohiggins")|>
      stringr::str_replace_all("antartica", "antartica chilena") |>
      stringr::str_replace_all("bio ?bio", "bio-bio") |>
      stringr::str_trim()
  )
...
reg_sf <- sf::st_as_sf(reg_terra) %>%
  mutate(
    region_clean = to_ascii_lower(NAME_1),
    # strip "region", "region de", "region del" prefixes if present
    region_clean = gsub("^region\\s+de\\s+", "", region_clean),
    region_clean = gsub("^region\\s+del\\s+", "", region_clean),
    region_clean = gsub("^region\\s+", "", region_clean)
  )

reg_counts$region_clean<- 
gsub("la araucania", "araucania", reg_counts$region_clean)
reg_counts$region_clean<- 
gsub("magallanes y de antartica chilena chilena","magallanes y de la antartica chilena", reg_counts$region_clean)
reg_counts$region_clean<- 
gsub("bio-bio","biobio", reg_counts$region_clean)
reg_counts$region_clean<- 
gsub("araucania","la araucania", reg_counts$region_clean)

# 3) Join your counts to the regions, compute rates
reg_sf <- mutate(reg_sf,region_clean = normalize_region(region_clean)) %>%
  dplyr::left_join(reg_counts, by = "region_clean") %>%
  dplyr::mutate(
...
unmatched <- reg_counts %>%
  dplyr::anti_join(reg_sf %>% sf::st_drop_geometry() %>% dplyr::select(region_clean), by = "region_clean")
print(unmatched)
...
reg_sf <- st_transform(reg_sf, crs = 4326)
...
cowplot::plot_grid(
  ...
  scale = 0.99             # reduce tamaño relativo de cada plot
)


if(grepl("ubuntu",system_r_platform())){
  ggsave(paste0(wdpath,"/_figs/map.png"), dpi = 600, scale=1)
} else{
  ggsave(paste0("_figs/map.png"), dpi = 600, scale=1)
}
