...
# --- The Flexible Matching Function ---
# Takes a vector of messy names and the master reference table
match_regions <- function(messy_names, reference_df) {
  
  # A. Create a function to pre-process/clean ANY string
  clean_string <- function(s) {
    # 1. Convert to lowercase
    s <- tolower(s)
    # 2. Remove accents (transliterate to basic ASCII)
    s <- iconv(s, from = "UTF-8", to = "ASCII//TRANSLIT")
    # 3. Remove punctuation and numbers
    s <- gsub("[[:punct:]]|[[:digit:]]", "", s)
    # 4. Remove common "stop words" that add no value
    s <- gsub("\\b(de|la|los|las|y|del|general|libertador|bernardo)\\b", "", s)
    # 5. Collapse multiple spaces into a single space
    s <- gsub("\\s+", " ", s)
    # 6. Trim whitespace from start and end
    s <- trimws(s)
    return(s)
  }
  
  # B. Apply the cleaning function to your messy input
  cleaned_messy_names <- sapply(messy_names, clean_string, USE.NAMES = FALSE)
  
  # C. Find the best match for EACH cleaned messy name
  matched_codes <- sapply(cleaned_messy_names, function(messy) {
    # Calculate the "distance" between the messy name and every clean name in our reference table
    # Method 'jw' (Jaro-Winkler) is great for short strings and typos
    distances <- stringdist::stringdist(messy, reference_df$clean_name, method = "jw")
    
    # Find which one is the closest (has the minimum distance)
    best_match_index <- which.min(distances)
    
    # Return the corresponding CUT code for the best match
    return(reference_df$roman_code[best_match_index])
  })
...
# Use the function to get the codes!
matched_cut_codes <- match_regions(CONS_C3_mod_c_a2$region_del_centro, master_regions)

# Add the new codes back to your original data frame
CONS_C3_mod_c_a2$CC1 <- matched_cut_codes
...
cowplot::plot_grid(
    ...
    scale = c(0.99, .99, 1.02) # reduce tamaño relativo de cada plot
)


if(grepl("ubuntu",system_r_platform())){
  ggsave(paste0("map_con_RR.png"), dpi = 600, scale=1)
} else{
  ggsave(paste0("map_con_RR.png"), dpi = 600, scale=1)
}
